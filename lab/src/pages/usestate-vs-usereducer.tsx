...
export default function useStateVSuseReducer() {
    ...
    switch (action) {
      case "change": {
        let newPassword: string =
          prompt("Введите новый пароль:  ") || "1234567890";
        return newPassword;
      }
      case "encrypt": {
        const newPassword: string = password2
          .split("")
          .map((value: string) => value.charCodeAt(0) ^ 1)
          .join(" ");
        return newPassword;
      }
      case "decode":
  ...
  const [password2, dispatchPassword] = useReducer(
    passwordReducer,
    "useReducer"
  );
  ...
  return (
    <>
      ...
      <p className="prose-m px-12 text-gray-500 md:px-0">
        In order to understand and feel the difference between these hooks and
        figure out which one is better to use and when, I will compare them for
        the same task
      </p>
      <br />
      <p className="prose-m px-12 text-gray-500 md:px-0">
        We have password. Let's realize the opportunity to change, encrypt or
        decipher it
      </p>
      <br />
      <br />
      ...
      <br />
      <a
        href="https://github.com/burydmitri/my-front-lab/tree/master/lab/src/pages/usestate-vs-usereducer.tsx"
      ...
      <p className="font-medium">
        UseReducer is an alternative to useState, they do the same thing.{" "}
      </p>
      <p className="font-medium">
        In huge components it can be more comfortable to see all variations of
        changing of state in one place - reducer. But in common cases i prefer
        useState, because, in my opinion useReducer is more cumbersome
      </p>
    </>
  );
}
